...
<head>
	...
	<script>
		function findPasswd() {
			...
			if ($("#mbr_nm").val() == "" || $("#phone").val() == "" || $("#email").val() == "") {
				alert("정보를 모두 입력해주세요!");
				return;
			}
			
			var data = {mbr_nm: $("#mbr_nm").val(), phone: $("#phone").val(), email: $("#email").val()};
			
			$.ajax({
				method: 'POST',
				url: '/front/member/findPasswdProc.json',
				...
				success:function(res) {
					if (res) {
						...
						alert("고객님의 전화번호로 인증번호를 보냈으니 확인해주세요");
					} else {
		...
		function changePasswd() {
			...
			var data = {mbr_nm: $("#mbr_nm").val(), phone: $("#phone").val(), email: $("#email").val(), passwd: $("#passwd").val()};
			
			$.ajax({
				method: 'POST',
				url: '/front/member/changePasswd.json',
				contentType: 'application/json',
				dataType: 'json', // 응답 데이터를 JSON으로 처리
				data: JSON.stringify(data),
				success:function(res) {
			...
			<form id="frmMain" name="frmMain" method="POST">
				...
				<div class="findPasswdArea">
					<ul class="memberInfo">
						<li>
							<span>이름</span>
							<input type="text" id="mbr_nm" name="mbr_nm" required placeholder="이름" autocomplete=off />
						</li>
						<li>
							<span>전화번호</span>
						...
						<li>
							<span>이메일</span>
							<input type="text" id="email" name="email" maxlength="32" required placeholder="이메일" autocomplete=off />
						</li>
					</ul>
					<ul class="chgpasswd">
						<li>
