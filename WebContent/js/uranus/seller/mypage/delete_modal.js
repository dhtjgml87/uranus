...
function validatePassword_(event) {
	...
	const seq_sll = document.getElementById('seq_sll').value; // 회원 번호
	const errorMessageDiv = document.getElementById('error-message'); // 오류 메시지 표시할 요소

	console.log("회원 번호:", seq_sll);
	...
	// 비밀번호 검증 요청
	fetch('/seller/sellers/passwd/deleteProc.web', {
		method: 'POST',
		headers: {
		'Content-Type': 'application/json'
		},
		body: JSON.stringify({ passwd: password, seq_sll: seq_sll }) // 비밀번호와 회원 번호 전송
	})
	.then(response => {
		if (response.ok) {
			// 비밀번호가 맞으면 두 번째 모달에 회원 번호와 비밀번호 설정
			document.getElementById('confirmSeq_sll').value = seq_sll; 
						console.log("회원 번호:", seq_sll);
...
function confirmDeletion(event) {
	...
	const seq_sll = document.getElementById('confirmSeq_sll').value; // 회원 번호
	...
	// 서버에 탈퇴 요청 보내기
	fetch('/seller/sellers/deleteProc.web', {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'
		},
		body: JSON.stringify({ seq_sll: seq_sll, passwd: password }) // 비밀번호와 회원 번호 전송
	})
	.then(response => {
		if (response.ok) {
