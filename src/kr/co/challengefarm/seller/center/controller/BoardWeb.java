 ...
 */
package kr.co.challengefarm.seller.center.controller;
...
import kr.co.challengefarm.seller.center.controller.BoardWeb;
import kr.co.challengefarm.seller.center.dto.BoardDto;
import kr.co.challengefarm.seller.center.dto.BoardFileDto;
import kr.co.challengefarm.seller.center.service.BoardSrvc;
import kr.co.challengefarm.seller.common.Common;
import kr.co.challengefarm.seller.common.component.SessionCmpn;
 ...
 * <p>IMPORTANT:</p>
 */
@Controller("kr.co.challengefarm.seller.center.controller.BoardWeb")
public class BoardWeb extends Common {
	...
	@RequestMapping(value = "/seller/center/board.web")
	public ModelAndView index(HttpServletRequest request, HttpServletResponse response, PagingDto pagingDto) {
		...
		try {
			...
			mav.setViewName("seller/center/board/notice/list");
		}
		catch (Exception e) {
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/download.web", method = RequestMethod.POST)
	public ModelAndView download(String type, long sequence, Model model) {
		...
		try {
			...
			if (type.equals("BbsNotice")) boardDto.setCd_bbs_type(4);
			else if (type.equals("BbsQuestion")) boardDto.setCd_bbs_type(6);
	...
	*/
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/remove.web", method = RequestMethod.POST)
	public ModelAndView remove(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto) {
		...
		try {
			...
			boardDto.setUpdater(Integer.parseInt(getSession(request, "SEQ_SLL")));
			...
			if (boardSrvc.deleteFlag(boardDto)) {
				...
				request.setAttribute("post"		, "/seller/center/board/list.web");
	...
	@RequestMapping(value = "/seller/center/board/modifyProc.web", method = RequestMethod.POST)
	public ModelAndView modifyProc(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto, FileUploadDto fileUploadDto) {
		...
		try {
			...
			String pathBase		= dynamicProperties.getMessage("seller.upload.path", "[UNDEFINED]") + "/bbs/";
			String maxSize		= dynamicProperties.getMessage("seller.upload.file.max10MB"			, "[UNDEFINED]");
			...
			if (message.equals("success")) {
				...
				*/
				...
				if (boardSrvc.update(boardDto)) {
					...
					// request.setAttribute("script"	, "alert('등록되었습니다.');");
					// request.setAttribute("redirect"	, "/seller/center/board/list.web?cd_bbs_type=" + boardDto.getCd_bbs_type());
				}
				else {
					request.setAttribute("script"	, "alert('시스템 관리자에게 문의하세요!');");
					request.setAttribute("redirect"	, "/");
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/modifyForm.web", method = RequestMethod.POST)
	public ModelAndView modifyForm(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto) {
		...
		try {
			...
			if (boardDto.getCd_bbs_type() == 6) {
				mav.setViewName("seller/center/board/question/modifyForm");
			}
			else {
				request.setAttribute("redirect"	, "/");
				mav.setViewName("forward:/servlet/result.web");
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/view.web", method = RequestMethod.POST)
	public ModelAndView view(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto,BoardFileDto boardFileDto) {
		...
		try {
			...
			if (boardDto.getCd_bbs_type() == 1) {
				mav.setViewName("seller/center/board/notice/view");
			}
			else if (boardDto.getCd_bbs_type() == 5) {
				mav.setViewName("seller/center/board/faq/view");
			}
				...
				mav.setViewName("seller/center/board/question/view");
			}
			else {
				request.setAttribute("redirect"	, "/");
				mav.setViewName("forward:/servlet/result.web");
	...
	@RequestMapping(value = "/seller/center/board/writeProc.web")
	public ModelAndView writeProc(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto, FileUploadDto fileUploadDto) {
		...
		try {
			...
			// 숫자열을 문자열인가 문자열을 숫자열인가 헷갈리면 이 코드 참고
			boardDto.setRegister(Integer.parseInt(getSession(request, "SEQ_SLL")));
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/writeForm.web")
	public ModelAndView writeForm(HttpServletRequest request, HttpServletResponse response, BoardDto boardDto) {
		...
		try {
			...
			//관리자 등록전까지 test 용
			if (boardDto.getCd_bbs_type() == 1) {
				mav.setViewName("seller/center/board/notice/writeForm");
			}
			else if (boardDto.getCd_bbs_type() == 5) {
				mav.setViewName("seller/center/board/faq/writeForm");
			}
			else  if (boardDto.getCd_bbs_type() == 6) {
				mav.setViewName("seller/center/board/question/writeForm");
			}
			else {
				request.setAttribute("redirect"	, "/");
				mav.setViewName("forward:/servlet/result.web");
	 ...
	 * <p>EXAMPLE:</p>
	 */
	@RequestMapping(value = "/seller/center/board/list.web", method = RequestMethod.POST)
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response, PagingDto pagingDto) {
		...
		try {
			...
			// 판매자 고객 문의(6)이면서 세션이 없을 경우는 로그인 페이지로 보냄
			if (pagingDto.getCd_bbs_type() == 6 && !sessionCmpn.isSession(request)) {
				request.setAttribute("script"	, "alert('로그인이 필요합니다!');");
				request.setAttribute("redirect"	, "/seller/login/loginForm.web?url=/seller/center/board/list.web");
				mav.setViewName("forward:/servlet/result.web");
			}
			else {
				
				if (pagingDto.getCd_bbs_type() == 6) pagingDto.setRegister(Integer.parseInt(getSession(request, "SEQ_SLL")));
				...
				if (pagingDto.getCd_bbs_type() == 1) {
					...
					mav.setViewName("seller/center/board/faq/list");
				}
				else if (pagingDto.getCd_bbs_type() == 6) {
					mav.setViewName("seller/center/board/question/list");
				}
				else {
					request.setAttribute("redirect"	, "/");
					mav.setViewName("forward:/servlet/result.web");
