 ...
 *				: [20241112152951][dhtjgml87#gmail.com][CREATE: Initial Release]
 */
package kr.co.challengefarm.seller.sales.service;
...
import kr.co.challengefarm.seller.sales.dao.SalesDao;
 ...
 * <p>IMPORTANT:</p>
 */
@Service("kr.co.challengefarm.seller.sales.service.SalesSrvc")
public class SalesSrvc {
	...
	public List<BuyDetailDto> month(PagingDto pagingDto) {
		return salesDao.month(pagingDto);
	}
	...
	public PagingListDto list(PagingDto pagingDto) {
		...
		// 총 매출
		int totalPrice = salesDao.price(pagingDto);
		
		// 전체 페이지 수 = 전체 라인(행) 수 / 페이징할 라인수
		int totalPage = (int) Math.ceil((double)totalLine / (double)pagingDto.getLinePerPage());
		pagingDto.setTotalLine(totalLine);
		pagingDto.setTotalPage(totalPage);
		pagingDto.setTotalPrice(totalPrice);
